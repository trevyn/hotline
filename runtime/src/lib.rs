...
impl Runtime {
    ...
    pub fn hot_reload(&mut self, lib_path: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        unsafe {
            ...
            // Re-register classes
            let classes_ptr = &mut self.classes as *mut HashMap<String, Box<dyn Fn() -> Box<dyn Object>>>;
            ...
            for (handle, (class_name, old_obj)) in self.objects.iter_mut() {
                if let Some(factory) = self.classes.get(class_name) {
                    // Create new object with same state
                    let mut new_obj = factory();
                    
                    // Copy state from old to new using getter/setter messages
                    // Get state from old object
                    let x = old_obj.receive(&Message { selector: "x".to_string(), args: vec![] });
                    let y = old_obj.receive(&Message { selector: "y".to_string(), args: vec![] });
                    let width = old_obj.receive(&Message { selector: "width".to_string(), args: vec![] });
                    let height = old_obj.receive(&Message { selector: "height".to_string(), args: vec![] });
                    
                    // Set state on new object
                    new_obj.receive(&Message { selector: "x:".to_string(), args: vec![x] });
                    new_obj.receive(&Message { selector: "y:".to_string(), args: vec![y] });
                    new_obj.receive(&Message { selector: "width:".to_string(), args: vec![width] });
                    new_obj.receive(&Message { selector: "height:".to_string(), args: vec![height] });
