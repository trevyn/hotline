                        ...
                        for px in 0..glyph_width {
                            ...
                            if src_offset + 1 < self.atlas.len() {
                                ...
                                if opacity > 0 {
                                    ...
                                    if screen_x >= 0 && screen_x < buffer_width as i32 && 
                                       screen_y >= 0 && screen_y < buffer_height as i32 {
                                        ...
                                        if dest_offset + 3 < buffer.len() {
                                            ...
                                            buffer[dest_offset + 1] = ((g as u32 * src_alpha + buffer[dest_offset + 1] as u32 * inv_alpha) / 255) as u8;
                                            buffer[dest_offset + 2] = ((r as u32 * src_alpha + buffer[dest_offset + 2] as u32 * inv_alpha) / 255) as u8;
                                            buffer[dest_offset + 3] = 255.min(buffer[dest_offset + 3] + src_alpha as u8);
                                        }
                                    }
                                }
                            }
